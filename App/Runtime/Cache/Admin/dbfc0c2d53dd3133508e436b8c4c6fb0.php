<?php if (!defined('THINK_PATH')) exit();?><h2 class="contentTitle">商品添加</h2>
      ...
      <div>
        <div class="pageFormContent nowrap" layoutH="158">
          <dl>
            <dt>商品名称：</dt>
            <dd>
              <input type="text" name="goods_name" maxlength="20" style="width:400px;" class="required" />
              <span class="info">请输入文章标题</span> </dd>
          </dl>
          <dl>
            <dt>商品简短描述：</dt>
            <dd>
              <input type="text" name="short_desc" style="width:300px;" />
          ...
          <dl>
            <dt>市场价：</dt>
            <dd>
              <input name="market_price" type="text" class="number" style="width:80px;" value="0" id="market_price" />
            </dd>
          </dl>
          <dl>
            <dt>商城价：</dt>
            <dd>
              <input name="shop_price" type="text" class="number" style="width:80px;" value="0" id="shop_price" />
            </dd>
          </dl>
          <dl>
            <dt>商品数量：</dt>
            <dd>
              <input name="goods_nums" type="text" class="digits" style="width:80px;" value="0" id="goods_nums" />
            </dd>
          </dl>
          <dl>
            <dt>商品关键词：</dt>
            <dd>
              <input type="text" name="keywords" style="width:400px;" />
            </dd>
          </dl>
          <dl>
            <dt>商品摘要：</dt>
            <dd>
              <textarea name="goods_brief" cols="80" rows="2" id="goods_brief"></textarea>
            </dd>
          </dl>
          <dl>
            <dt>缩略图：</dt>
            <dd>
              <input name="goods_thumb" id="goodsadd_attachmentid" value="" type="hidden">
              <div id="goodsadd_attach"></div>
              <span class="info"></span> </dd>
          </dl>
          <dl>
            <dt>图片预览：</dt>
            <dd> <img name="thumbimg" id="goodsadd_thumbimg" src="__PUBLIC__/images/no_picture.jpg" /> </dd>
          </dl>
        </div>
      </div>
      <!--详细描述-->
...
upMediaUrl="<?php echo U('Attach/editor_mediaupload');?>" upMediaExt:"avi">
          ...
          <dl>
            ...
            <dd>
              <label style="width:80px;">
                <input name="online_sale" type="radio" value="1" checked="checked" />
                上架</label>
              <label style="width:80px;">
                <input type="radio" value="0" name="online_sale" />
                下架</label>
            </dd>
          </dl>
          <dl>
            ...
            <dd>
               <label style="width:80px;">
                ...
                推荐</label>
              <label style="width:80px;">
                <input name="is_recommend" type="radio" value="0" checked="checked" />
                不推荐</label>
            </dd>
          </dl>
          <dl>
            <dt>商品已售数量：</dt>
            <dd>
              <input type="text" class="digits" name="sale_num" value="0" /></dd>
          </dl>
          <dl>
            <dt>商品人气：</dt>
            <dd>
              <input type="text" class="digits" name="pop_num" value="0" />
              <span class="info"></span></dd>
          </dl>
          <dl>
            <dt>是否新品：</dt>
            <dd>
              <label style="width:80px;">
                ...
                推荐</label>
              <label style="width:80px;">
                <input name="is_new" type="radio" value="0" checked="checked" />
                不推荐</label>
            </dd>
          </dl>
           <dl>
            ...
            <dd>
              <label style="width:80px;">
                ...
                新品</label>
              <label style="width:80px;">
                <input name="is_hot" type="radio" value="0" checked="checked" />
                非新品</label>
            </dd>
          </dl>
          <dl>
            ...
            <dd>
              <label style="width:80px;">
                ...
                精品</label>
              <label style="width:80px;">
                <input name="is_best" type="radio" value="0" checked="checked" />
                非精品</label>
            </dd>
          </dl>
        </div>
      ...
      <div>
        ...
        <dl>
          <dt>所属类型:</dt>
          <dd>
            <select name="goods_type" id="goodsadd_type" class="combox" onchange="javascript:showAttrList();">
        ...
        <div id="goodsadd_type_box">
        </div>
        </div>
      </div>
      <!--商品相册-->
      <div>
        <div id="goodsadd_album"></div>
        <input type="hidden" name="album" id="goodsadd_albumlist" value="" />
        <div id="goodsadd_alblum_box"> </div>
      </div>
    </div>
    <div class="formBar">
      <ul>
...
$(document).ready(function() {
	var goodsadduploader	=	new	qq.FineUploader({
		element: $('#goodsadd_attach')[0],
		request:{
		...
		callbacks:{
			onComplete: function(id, fileName, responseJSON ){
					if (responseJSON.success) {
						$("#goodsadd_attachmentid").val( responseJSON.id );
						$("#goodsadd_thumbimg").attr("src" , responseJSON.attachmentPath);
					}
				}
			},
		debug: true
		});
	var goodsadd_album_uploader = new qq.FineUploader({
		element: $('#goodsadd_album')[0],
		request:{
			endpoint:"<?php echo U('Attach/artuploadimg');?>",
			forceMultipart: true,
			inputName: 'album'
...
function update_alblum()
{
	...
	$("#goodsadd_alblum_box input").each(function(index, element) {
        alblum_list[ index ]	=	$(this).val();
    });
	$("#goodsadd_albumlist").val( alblum_list.join(',') );
}
//显示商品属性列表
function showAttrList(goods_id)
{
	...
	{
		goods_id:goods_id,
		goods_type:$("#goodsadd_type").val()
	},
	function(text){
		$("#goodsadd_type_box").html( text );
	},"text"
	);*/
	$("#goodsadd_type_box").load(
	'<?php echo U("Goods/ArributeList");?>',
	{
		goods_id:goods_id,
		goods_type:$("#goodsadd_type").val()
